...
send_transcript_to_waifu() {
	...
	--header 'x-rapidapi-key: 1ad22d441fmsh6e3b6b479272ff4p18915fjsn16204f31152f' \
	--data '{"model":"gpt-3.5-turbo","messages":[{"role":"user","content":"$transcript"}]}'
) 
    # Display the waifu response
    echo "Waifu Response:"
    #echo "$waifu_response2"
      # Extract message using jq (assuming successful response)
  if [[ $? -eq 0 ]]; then
    message=$(echo "$waifu_response" | jq -r '.message.content')
    echo "Waifu Response:"
    echo "$message"
  else
    echo "Error: Failed to retrieve message from JSON."
  fi
...
play_synthesized_speech() {
    ...
    #aplay "$polly_output_file"
}
# Clean up sound files from AWS S3 bucket and local folder
count(){
message_length=${#message}
Duration=$(echo "$message_length / 24000 * 60" | bc)
echo "$message_length"
